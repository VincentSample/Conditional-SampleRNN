...
class GeneratorPlugin(Plugin):
    ...
    def __init__(self, samples_path, n_samples, sample_length, sample_rate, nb_classes, cond = False):
        super().__init__([(1, 'epoch')])
        ...
        self.cond = cond
    def register(self, trainer):
        self.generate = Generator(trainer.model.model, self.nb_classes, trainer.cuda)
    def register_generate(self, model, cuda):
        self.generate = Generator(model, self.nb_classes,cuda,self.cond)
    def epoch(self, epoch_index):
        samples = self.generate(self.n_samples, self.sample_length) \
                      .cpu().float().numpy()
        for i in range(self.n_samples*self.nb_classes):
            write_wav(
                os.path.join(
